...
基于 Dolby Encoding Engine 的 **Dolby Vision Profile 7 双层分布式编码工作流**。本项目在 Dolby 官方 `dv_profile_7_workflow_chunked.py` 示例脚本基础上，针对 UHD 蓝光编码场景进行了四轮递进式增强——从独立 YUV 清理工具、清理逻辑集成、MEL 性能优化到全局进度监控，最终形成一套面向实际生产环境的完整编码管线。
...
```
DEE_Workflow/
├── Encoder.bat                  ← 一键启动脚本（当前版本 V3，含全部优化参数）
...
本项目经历了四个迭代版本，每一版都精准解决了上一版遗留的核心痛点，沿着 **"安全 → 高效 → 可观测"** 的主线持续演进：
...
YUV 清理机制、MEL 优化和进度监控等增强功能由 **LumaVista** 开发。
